    ...
    <table id="templates_table" class="table table-hover table-bordered" width="100%">
        ...
        <tbody>
            ...
            @foreach($templates as $template)
                ...
                @endphp
                <tr>
                    <td><a href="/esign/esign_add_fields/0/yes/{{ $template -> id }}" class="btn btn-primary" target="_blank">Open <i class="fal fa-arrow-right ml-2"></i></a></td>
                    <td>{{ $template -> template_name }}</td>
                    <td>{!! implode(', ', $recipients) !!}</td>
                    <td>{{ date('M jS, Y', strtotime($template -> created_at)) }}</td>
                    <td class="text-center"><a href="javascript:void(0)" class="btn btn-danger delete-template-button" data-envelope-id="{{ $template -> id }}"><i class="fal fa-times"></i></a></td>
                </tr>
            @endforeach
